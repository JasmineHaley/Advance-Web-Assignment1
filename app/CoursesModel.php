...
class CoursesModel extends Observable_Model {
	...
	for($i = 1;$i <= count($dept_courses["faculty_dept_courses"]);$i++){
		...
		for($j =0; $j < count($dept_courses["faculty_dept_courses"][$i]);$j++){
			...
			$c[]=  array($dept["faculty_department"][$i],$courses["courses"][$dept_courses["faculty_dept_courses"][$i][$j]][0],$courses["courses"][$dept_courses["faculty_dept_courses"][$i][$j]][4],$instructor["instructors"][$course_instructor["course_instructor"][$i]]);
		}
	}
	$info = array_merge($c,$info);
